...
namespace CazadorDeTesoros
{
    public class CazadorDeTesoros
    {
        ...
        public const int LLAVE = 6; // NUEVO: Código para la llave
        ...
        private List<Tesoro> inventario; // Inventario de tesoros
        private bool tieneLlave; // NUEVO: Estado de la llave
        ...
        // NUEVO: Temporizador para recompensas por tiempo
        ...
        public CazadorDeTesoros()
        {
            ...
            this.tieneLlave = false; // NUEVO: Inicialmente sin llave
            this.ranking = new List<Jugador>();
            this.random = new Random();
            this.temporizador = new Stopwatch(); // NUEVO: Inicializar temporizador
        ...
        public void GuardarPartida()
        {
            try
            {
                using (StreamWriter writer = new StreamWriter(ARCHIVO_PARTIDA))
                {
                    ...
                    writer.WriteLine($"{vidas},{energia},{puntaje},{movimientosSinChocar},{posX},{posY},{posZ},{tieneLlave}");
                    
                    // Guardar tiempo transcurrido
                    writer.WriteLine(temporizador.ElapsedMilliseconds);
        ...
        public bool CargarPartida()
        {
            ...
            try
            {
                ...
                tieneLlave = bool.Parse(stats[7]); // NUEVO: Cargar estado de llave
                
                // Cargar tiempo transcurrido
                long tiempoTranscurrido = long.Parse(lineas[lineaIndex++]);
                temporizador.Restart();
                temporizador.Start();
                // Ajustar el tiempo transcurrido
                System.Threading.Thread.Sleep(1); // Pequeña pausa para evitar 0
        ...
        // NUEVO: Algoritmo mejorado para generar laberintos sin caminos sin salida
        private void GenerarLaberintoMejorado(int nivel)
        {
            ...
            // Crear pasillos usando algoritmo de profundidad con backtracking
            Stack<Tuple<int, int>> pila = new Stack<Tuple<int, int>>();
            int startX = 1;
            int startY = 1;
            
            tablero[startX, startY, nivel] = VACIO;
            pila.Push(new Tuple<int, int>(startX, startY));
            ...
            while (pila.Count > 0)
            {
                Tuple<int, int> actual = pila.Peek();
                int x = actual.Item1;
                int y = actual.Item2;
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    if (nx > 0 && nx < ANCHO - 1 && ny > 0 && ny < ALTO - 1 && tablero[nx, ny, nivel] == PARED)
                    {
                        // Verificar que no estamos creando un área cerrada
                        if (EsDireccionValida(nx, ny, nivel, dx[i], dy[i]))
                        {
                            direcciones.Add(i);
                        }
                    }
                ...
                {
                    int dir = direcciones[random.Next(direcciones.Count)];
                    int nx = x + dx[dir] * 2;
                    int ny = y + dy[dir] * 2;
                    
                    // Crear pasillo
                    tablero[x + dx[dir], y + dy[dir], nivel] = VACIO;
                    tablero[nx, ny, nivel] = VACIO;
                    
                    pila.Push(new Tuple<int, int>(nx, ny));
                }
                else
                {
                    pila.Pop();
            ...
            // Asegurar conectividad adicional
            AsegurarConectividad(nivel);
            
            // Crear bordes
            for (int x = 0; x < ANCHO; x++)
            {
                tablero[x, 0, nivel] = PARED;
                tablero[x, ALTO - 1, nivel] = PARED;
            }
            for (int y = 0; y < ALTO; y++)
            {
                tablero[0, y, nivel] = PARED;
                tablero[ANCHO - 1, y, nivel] = PARED;
            }
        }
        
        // NUEVO: Método para verificar si una dirección es válida
        private bool EsDireccionValida(int x, int y, int nivel, int dx, int dy)
        {
            // Contar espacios vacíos alrededor de la posición objetivo
            int vacios = 0;
            for (int i = -1; i <= 1; i++)
            {
                for (int j = -1; j <= 1; j++)
                {
                    ...
                    if (nx >= 0 && nx < ANCHO && ny >= 0 && ny < ALTO && tablero[nx, ny, nivel] == VACIO)
                    {
                        vacios++;
                    }
                }
            ...
            // Permitir conexión si no crea un área demasiado cerrada
            return vacios <= 3;
        }
        
        // NUEVO: Método para asegurar conectividad del laberinto
        private void AsegurarConectividad(int nivel)
        {
            // Conectar áreas desconectadas eliminando algunas paredes estratégicamente
            for (int i = 0; i < 20; i++)
            {
                ...
                if (tablero[x, y, nivel] == PARED)
                {
                    ...
                    // Convertir en pasillo si conecta al menos 2 áreas
                    if (vaciosAdyacentes >= 2)
                    {
        ...
        private void ColocarElementos(int nivel)
        {
            ...
            // NUEVO: Colocar llave en cada nivel
            ColocarElementoAleatorio(LLAVE, nivel);
        ...
        private void ColocarElementoAleatorio(int elemento, int nivel)
        {
            ...
            do
            {
                ...
                intentos++;
            } while (tablero[x, y, nivel] != VACIO && intentos < 100);
        ...
        public void ImprimirTablero()
        {
            ...
            Console.WriteLine($"Inventario: {inventario.Count}/{CAPACIDAD_INVENTARIO} tesoros | Llave: {(tieneLlave ? "SÍ" : "NO")}");
            // NUEVO: Mostrar tiempo transcurrido
            Console.WriteLine($"Tiempo: {temporizador.Elapsed:mm\\:ss}");
            MostrarInventario();
            Console.WriteLine();
                ...
                for (int x = 0; x < ANCHO; x++)
                {
                    ...
                    else
                    {
                        switch (tablero[x, y, posZ])
                        {
                            ...
                            case LLAVE: simbolo = 'K'; break; // NUEVO: Símbolo para llave
        ...
        private void ProcesarCelda(int x, int y)
        {
            ...
            switch (celda)
            {
                ...
                case SALIDA:
                    UsarSalida();
                    break;
                case LLAVE: // NUEVO: Procesar llave
                    RecogerLlave(x, y);
                    break;
            }
        }
        ...
        {
            vidas--;
            // NUEVO: Quitar 100 puntos al activar trampa
            puntaje = Math.Max(0, puntaje - 100);
            Console.WriteLine($"¡Caíste en una trampa! Vidas: {vidas} | Se restaron 100 puntos");
            if (vidas <= 0)
            {
                GameOver();
            }
        ...
        // NUEVO: Método para recoger llave
        private void RecogerLlave(int x, int y)
        {
            if (!tieneLlave)
            {
                tieneLlave = true;
                Console.WriteLine("¡Encontraste la llave! Ahora puedes usar la salida.");
                tablero[x, y, posZ] = VACIO;
            }
            else
            {
                Console.WriteLine("Ya tienes una llave.");
            }
        }
        ...
        private void UsarSalida()
        {
            if (!tieneLlave)
            {
                Console.WriteLine("¡Necesitas encontrar la llave primero para usar la salida!");
                return;
            }
            ...
            if (posZ < NIVELES - 1)
            {
                ...
                // Recompensa fija de 100 puntos por pasar nivel
                int recompensa = 100;
                puntaje += recompensa;
                Console.WriteLine($"¡Recompensa por completar nivel: {recompensa} puntos!");
                
                // Reiniciar estado de llave para el nuevo nivel
                tieneLlave = false;
        ...
        // NUEVO: Método para calcular recompensa por tiempo
        private int CalcularRecompensaPorTiempo()
        {
            // Detener el temporizador
            temporizador.Stop();
            
            // Calcular recompensa basada en el tiempo (más rápido = más puntos)
            double segundos = temporizador.Elapsed.TotalSeconds;
            int recompensaBase = 1000; // Recompensa base por completar el juego
            
            // Fórmula: recompensa = base + (3000 / tiempo_en_segundos)
            // Esto significa que jugadores más rápidos obtienen más puntos
            int recompensaTiempo = (int)(3000 / Math.Max(1, segundos / 60)); // Normalizar a minutos
            
            return recompensaBase + recompensaTiempo;
        }
        
        private void GameOver()
        {
            // NUEVO: Calcular recompensa por tiempo
            int recompensaTiempo = CalcularRecompensaPorTiempo();
            puntaje += recompensaTiempo;
            
            Console.WriteLine($"\n¡Recompensa por tiempo: {recompensaTiempo} puntos!");
            
            // Asegurar que el puntaje sea múltiplo de 100
        ...
        public void Jugar()
        {
            ...
            Console.WriteLine("Encuentra la llave (K) para abrir la salida");
            Console.WriteLine("Las trampas quitan 100 puntos");
            Console.WriteLine("Recompensa por tiempo - ¡Juega más rápido para más puntos!");
