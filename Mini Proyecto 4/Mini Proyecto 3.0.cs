...
namespace CazadorDeTesoros
{
    public class CazadorDeTesoros
    {
        ...
        private void AgregarAlRanking()
        {
            if (puntaje > 0)
            {
                Console.WriteLine("\n¡Has conseguido un puntaje alto!");
        ...
        {
            // Revelar la posición actual y las adyacentes
        ...
        private void UsarSalida()
        {
            if (posZ < NIVELES - 1)
            {
                // Convertir todos los tesoros del inventario a puntos antes de cambiar de nivel
                if (inventario.Count > 0)
                {
